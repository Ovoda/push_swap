...
t_var	init_var(t_stack **a, t_stack **b, int index_b, int pivot)
{
	t_var	var;
	...
	var.value_a = get_top_a(*a, var.value_b, pivot);
		...
		int pivot_index[2], int print)
{
	...
	var = init_var(a, b, pivot_index[1], pivot_index[0]);
...
int		count_steps(t_stack **stack, t_stack **b, int pivot_index[2])
{
	...
	tmp_b = sl_stackdup(*b);
	ret = count_step_to_get_right_pos(&tmp_a, &tmp_b, pivot_index, 0);
	sl_free_stack(tmp_a);
	sl_free_stack(tmp_b);
	return (ret);
}
...
int		try_all_elems(t_stack **stack, t_stack **b, int pivot)
{
	...
	{
		min[1] = count_steps(stack, b, pivot_index);
	...
	pivot_index[1] = min[2];
	count_step_to_get_right_pos(stack, b, pivot_index, 1);
	return (min[0]);
}
